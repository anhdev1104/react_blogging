const ImageUpload = props => {
  ...
  return (
    ...
    >
      ...
      {progress !== 0 && !image && (
        <div className="absolute h-16 w-16 border-8 border-green-500 border-t-transparent animate-spin rounded-full "></div>
      )}
      {!image && progress === 0 && (
      ...
      {image && (
        <>
          ...
          <button
            type="button"
            className="w-16 h-16 bg-white rounded-full flex items-center justify-center cursor-pointer absolute z-10 text-red-500 duration-300 transition-all opacity-0 invisible group-hover:opacity-100 group-hover:visible"
